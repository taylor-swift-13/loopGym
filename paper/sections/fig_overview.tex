...
%% ---------------- panel (b): sampler ----------------
\node[anchor=north west] (btitle) at (4.9,5.6) {\textbf{(b) Sampler: positives + candidate negatives}};
\begin{scope}[shift={(5.7,1.55)}]
  ...
  \node[lbl, text=codered, anchor=east] at (4.25,1.72) {escape: outside};
  \node[lbl, text=codered, anchor=east] at (4.25,1.02) {sampled range};
...
\node[neg, anchor=west] (f3) at ($(f2.east)+(0.18,0)$) {oracle?};
\node[lbl, anchor=west] at ($(f3.east)+(0.15,0)$) {$\Rightarrow$ veto};
\node[lbl, anchor=north west] at (5.35,-0.25) {trace units: one synthetic history $=$ one unit};

%% ---------------- panel (c): reward ----------------
\node[anchor=north west] (ctitle) at (10.9,5.6) {\textbf{(c) Minimal dense reward}};
...
\node[stage, anchor=north west, align=left] (casc) at ($(roll.south west)+(0,-0.3)$)
  {positive filter $\to$ syntax\\ $\to$ Frama-C/WP Houdini};
...
\node[box, anchor=north west, fill=white] (score) at ($(casc.south west)+(0,-0.35)$)
  {$r = \tfrac12\,\mathrm{base} + \tfrac12\,\mathrm{marginal}$};
\node[chip, anchor=north west] (s0) at ($(score.south west)+(0.05,-0.3)$) {base: $A$ alone};
\node[chip, anchor=west] (s1) at ($(s0.east)+(0.12,0)$) {marginal: union ablation};
\draw[arr] (roll) -- (casc);
\draw[arr] (casc) -- (score);
\draw[arr] ($(score.south)+(0,-0.05)$) -- ($(score.south)+(0,-0.28)$);
\draw[arr] (s1.east) -- ++(0.35,0) node[anchor=west, lbl, align=left] {RL\\ update};
\end{tikzpicture}
}%
\caption{\textbf{\sam{} overview.}
(a)~The policy sees precondition, guard, and body; any assertion present is stripped in closed-book mode, and the policy must emit the \emph{strongest} inductive loop summary rather than an invariant tailored to a target.
(b)~The sampler executes the instrumented loop over a deterministic, precondition-respecting input sweep; positives are observed loop-head states, and negative candidates come from relational perturbations around dense trace windows, over-run continuations past a genuine exit, and sampled-range escapes.
Reachability and entry-feasibility guards remove contradicted labels; incomplete state/control coverage disables synthetic negatives, and a broader-run audit checks for pair-level collisions.
(c)~Each rollout is reduced to its surviving inductive core and receives the equal-weighted sum of its standalone rejection rate and its ablation-based marginal contribution; this is the complete scoring formula.}
\label{fig:overview}
\end{figure}
