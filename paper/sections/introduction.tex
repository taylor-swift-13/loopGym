...
CEGIS- and ICE-style learners derive their counterexamples from failed attempts to prove $Q$~\citep{garg2014ice,garg2016learning,padhi2016pie}; Code2Inv shapes its reinforcement signal from the solver's rejection of $Q$-directed verification conditions~\citep{si2018learning,si2020code2inv}; LLM pipelines put the assertion in the prompt and score candidates by whether the verifier closes the goal~\citep{kamath2023finding,chakraborty2023ranking,wu2024lemur}.
...
We instead study the task of generating an inductive \emph{loop summary}: given only the precondition, guard, and body---\emph{no postcondition}---produce the strongest practically expressible inductive invariant characterizing the loop's reachable states at the loop head.
...
What must be rewarded is \emph{soundness plus tightness}: how much of the unreachable state space does the candidate provably exclude?
We operationalize this with an execution-grounded dense reward.
A sampler compiles and runs the instrumented loop over a deterministic input sweep to collect \emph{positive} loop-head states, then constructs \emph{truthful negatives}---trace units that are provably impossible under the loop's own dynamics---along two balanced dimensions: a \emph{relational} dimension (small perturbations off densely observed trace neighborhoods, exposing missing laws such as $x + y = n$) and a \emph{bound} dimension (post-exit continuations of real traces and box-escape states, exposing missing range facts).
...
Such a reward is a textbook target for reward hacking~\citep{amodei2016concrete,skalse2022defining,gao2023scaling}: a policy can memorize the sampled negative pool with sprays of disequalities, or fit sample-extreme bounding boxes, without learning anything semantic.
We treat hack resistance as a first-class design requirement and harden the reward with a per-invariant complexity gate, a per-rollout atom budget, unique-kill junk discounting, and---crucially---multi-seed \emph{minimum} scoring against held-out sampling seeds whose input envelopes are decorrelated by seed-hashed far probes and tier jitter, so that any predicate fitted to one concrete sample collapses on the others while true invariants are seed-indifferent.
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, topsep=2pt]
  ...
  \item \textbf{An execution-grounded, hack-resistant reward.} We design a dense reward for this task built on truthful negatives---provably impossible traces constructed from the loop's own dynamics along relational and bound dimensions---with Houdini-pruned survival, trace-unit accounting, and a layered anti-hacking defense: complexity gate, atom budget, junk discounting, holdout-seed minimum scoring, far probes, tier jitter, slice caps, and dimension balancing ($\S$\ref{sec:method}).
  \item \textbf{An open three-component pipeline.} \sam{} decomposes into an example sampler, a containerized HTTP reward service consumable by any RL trainer, and an inference framework with Houdini repair and Frama-C/WP certification ($\S$\ref{sec:overview}).
  \item \textbf{Adversarial validation of the reward.} Three regression gates---a discrimination suite of gold/loose/adversarial invariant families, a gold-free hack sweep, and a mislabel audit over all generated negatives---pass with zero violations on 366 linear and nonlinear benchmark programs ($\S$\ref{sec:experiments}).
\end{itemize}
