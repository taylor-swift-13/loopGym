...
We treat hack resistance as a first-class design requirement and harden the reward with a per-invariant complexity gate, a per-rollout atom budget, unique-kill junk discounting, decade-spread seed-hashed negative placement (defeating both pointwise and interval-window memorizers), and---crucially---multi-seed \emph{minimum} scoring against held-out sampling seeds whose input envelopes are decorrelated by seed-hashed far probes and tier jitter, so that any predicate fitted to one concrete sample collapses on the others while true invariants are seed-indifferent.
