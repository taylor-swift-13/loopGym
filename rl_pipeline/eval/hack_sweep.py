...
def gates_for(n_neg: int, diseq_bound: float):
    # diseq is gated by CONSISTENCY: a 32-atom farm can provably kill at most
    # the top-32 farm-targetable slices of the pool (diseq_bound); exceeding
    # that means a slice-cap leak in the sampler.  On small finite state
    # spaces diseq_bound is legitimately ~1 (complement enumeration is crude
    # TRUTH there, and the production Houdini drops non-inductive diseqs).
    return {
        "trivial": 0.01,
        "mega_conj": 0.01,
...
def sweep_one(rel: str, n_runs: int) -> dict:
    ...
    from .discrimination import (HOLDOUT_SEED, _MergedExamples, chunked_conjunction,
                                 const_bounds, diseq_farm, mega_conjunction,
                                 state_memorizer)
    ...
    try:
        ...
        gates = {} if low_signal else gates_for(n_neg, diseq_bound)
        ...
        return {"program": rel, "n_neg": n_neg, "low_signal": low_signal,
                "diseq_bound": round(diseq_bound, 3),
...
def main() -> int:
    ...
    for fam in ("trivial", "mega_conj", "state_mem", "chunked_conj", "diseq_farm64") + REPORT_ONLY:
